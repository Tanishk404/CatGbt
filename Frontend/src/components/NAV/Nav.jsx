  ...
  return (
    <div className='w-full pr-7 bg-gray-200 dark:bg-[#1E1E1E] mt-0 p-2 flex h-16 justify-between z-40 absolute lg:static md:static xl:static'>
...

        {
          ...
          <Link to={"/user/login"} className='text-xs hover:bg-[#2e2e2f] bg-black text-white h-6 text-center justify-center flex p-2 rounded-2xl items-center' >Login</Link> 
          <Link className='text-xs dark:hover:bg-[#2e2e2f] border border-gray-500 rounded-2xl text-center hover:bg-gray-300 justify-center items-center w-24 flex h-6 p-2 font-bold' to={"/user/signup"}>Sign up</Link>
          </div>)
        }
    </div>
  )

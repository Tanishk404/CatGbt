...
function SearchChat({searchChat, setSearchChat}) {
  ...
  return (
    <div className={clsx('bg-[rgb(236,233,233)] overflow-y-hidden z-40 w-full max-w-[30rem] absolute overflow-hidden h-full md:max-h-[25rem] shadow-lg lg:max-h-[25rem] mt-20 rounded-lg pb-10 p-1',
        ...
        <div className='w-full text-start h-[100%] lg:h-80 md:h-80 xl:h-80 overflow-y-auto flex flex-col p-4'>
          ...
          data.map((v,i) => {
              return(
                ...
                <Link key={v._id} to={`chat/${v._id}`} className='mt-6 hover:bg-slate-100 p-1 rounded-lg'>{v.title}</Link>
